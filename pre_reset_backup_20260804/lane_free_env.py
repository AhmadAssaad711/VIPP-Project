...
class LaneFreeTrafficEnv(AbstractEnv):
        ...
        config.update(
            {
                ...
                # ``full`` exposes vehicle dimensions; ``minimal`` keeps only
                # relative position, velocity, and desired speed.  The
                # simulator still uses physical dimensions for collisions.
                "observation_mode": "full",
                # MTM desired speeds can reach 25 m/s * 1.15 for aggressive
                # profiles.  Use a scale that covers that range while keeping
                # the learned observation contract finite and normalized.
                "observation_vmax": 30.0,
                "observation_vymax": 9.0,
                "force": {
                    "k_target_x": 2.5,
                    "k_target_y": 2.0,
                    "k_rep": 8.0,
    ...
    def define_spaces(self) -> None:
        ...
        features = rows * len(self._observation_feature_names())
    ...
    def _observation_feature_names(self) -> tuple[str, ...]:
        mode = str(self.config.get("observation_mode", "full")).strip().lower()
        if mode == "full":
            return ("dx", "dy", "vx", "vy", "length", "width", "desired_speed")
        if mode == "minimal":
            return ("dx", "dy", "vx", "vy", "desired_speed")
        raise ValueError(
            "observation_mode must be either 'full' or 'minimal', "
            f"received {mode!r}"
        )
    ...
    def step(self, action: np.ndarray | list[float] | None) -> tuple[np.ndarray, float, bool, bool, dict[str, Any]]:
        ...
        # Collision diagnostics are emitted once per policy step, while the
        # simulator may integrate several physics frames inside that step.
        # Accumulate event onsets and active ego contact across all frames so
        # the reward, termination, and KPI layers observe the same result.
        ...
        self._last_collision_count = int(policy_collision_count)
        self._last_active_collision_count = int(policy_active_collision_count)
        self._last_ego_collision_count = int(policy_ego_collision_count)
        self._last_ego_collision = bool(policy_ego_collision)
    ...
    def _observe(self) -> np.ndarray:
        ...
        feature_names = self._observation_feature_names()
        rows = np.zeros(
            (1 + neighbor_count, len(feature_names)),
            dtype=np.float32,
        )
        if not vehicles:
        ...
        observation_vmax = max(float(self.config.get("observation_vmax", 30.0)), 1e-6)
        observation_vymax = max(float(self.config.get("observation_vymax", 0.3 * observation_vmax)), 1e-6)
        normalized_columns = {
            "dx": np.clip(signed_dx / sensing_range, -1.0, 1.0),
            "dy": np.clip(relative_y / road_width, -1.0, 1.0),
            "vx": np.clip(vx / observation_vmax, -1.0, 1.0),
            "vy": np.clip(vy / observation_vymax, -1.0, 1.0),
            "length": np.clip(lengths / 5.15, -1.0, 1.0),
            "width": np.clip(widths / 1.84, -1.0, 1.0),
            "desired_speed": np.clip(desired_speeds / observation_vmax, -1.0, 1.0),
        }
        for column, feature_name in enumerate(feature_names):
            rows[:selected_count, column] = normalized_columns[feature_name][selected]
        rows[0, :2] = 0.0
        return rows.reshape(-1)
    ...
    def _observation_row(self, vehicle: LaneFreeVehicle, ego: LaneFreeVehicle) -> np.ndarray:
        ...
        normalized_values = {
            "dx": np.clip(signed_dx / sensing_range, -1.0, 1.0),
            "dy": np.clip(dy / road_width, -1.0, 1.0),
            "vx": np.clip(vehicle.vx / observation_vmax, -1.0, 1.0),
            "vy": np.clip(vehicle.vy / observation_vymax, -1.0, 1.0),
            "length": np.clip(vehicle.length / 5.15, -1.0, 1.0),
            "width": np.clip(vehicle.width / 1.84, -1.0, 1.0),
            "desired_speed": np.clip(vehicle.desired_speed / observation_vmax, -1.0, 1.0),
        }
        return np.asarray(
            [normalized_values[name] for name in self._observation_feature_names()],
            dtype=np.float32,
        )
