...
def evaluate_candidate(
    ...
    for episode in range(int(episodes)):
        ...
        # Use the simulator's per-step ``*_events`` counters accumulated
        # above.  They count distinct *new* collision-pair transitions.  Do
        # not replace them with the notebook KPI wrapper's cumulative ego
        # counter: that wrapper can fall back to one while an already-active
        # ego collision persists, which would count one pair repeatedly and
        # can make ego events exceed all collision-pair events.
        rows.append(
            {
        ...
        json.dumps(
            {
                ...
                "ego_collisions_per_km_definition": "distinct new ego-involved collision-pair events divided by ego distance traveled",
                "total_collisions_per_km_definition": "distinct new collision-pair events divided by ego distance traveled",
            },
            indent=2,
        ),
        encoding="utf-8",
